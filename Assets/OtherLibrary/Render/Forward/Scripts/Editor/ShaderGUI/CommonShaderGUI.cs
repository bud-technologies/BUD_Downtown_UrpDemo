    ...
    public class CommonShaderGUI : ShaderGUI
    {
        ...
        protected void DrawBlendProp(Material material)
        {
            ...
            if (m_BlendOpProp!=null)
            {
                ...
                DoPopup(m_BlendOpText, m_BlendOpProp, Enum.GetNames(typeof(NewBlendOp)));
            }
        ...
        protected enum ZWriteType
        {
            On = 1,
            Off = 0,
        }

        protected MaterialProperty m_ZWriteProp;
        protected static GUIContent m_ZWriteText = new GUIContent("深度写出", "开启/关闭");

        protected void DrawZWriteProp(Material material)
        {
            m_ZWriteProp = FindMaterialProperty("_ZWrite");
            if (m_ZWriteProp != null)
            {
                //Render face
                DoPopup(m_ZWriteText, m_ZWriteProp, Enum.GetNames(typeof(ZWriteType)));
            }
        }
        ...
        public void DrawVectorSlider(MaterialProperty prop,bool indexA, bool indexB, bool indexC, bool indexD, 
            Vector2 rangeA, Vector2 rangeB, Vector2 rangeC, Vector2 rangeD,
            string newNameA, string newNameB, string newNameC,string newNameD)
        {
            if (prop == null) return;
            ...
            if (indexA)
            {
                vectorValues.x = EditorGUILayout.Slider(newNameA,vectorValues.x, rangeA.x, rangeA.y);
            }
            if (indexB)
            {
                vectorValues.y = EditorGUILayout.Slider(newNameB, vectorValues.y, rangeB.x, rangeB.y);
            }
            if (indexC)
            {
                vectorValues.z = EditorGUILayout.Slider(newNameC, vectorValues.z, rangeC.x, rangeC.y);
            }
            if (indexD)
            {
                vectorValues.w = EditorGUILayout.Slider(newNameD, vectorValues.w, rangeD.x, rangeD.y);
            }
            prop.vectorValue = vectorValues;
        }
