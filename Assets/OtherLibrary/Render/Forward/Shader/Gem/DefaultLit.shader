...
Shader "NewRender/Gem/DefaultLit" {

    Properties{
        [NoScaleOffset] _RefractMap("CubeMap", Cube) = "" {}
        [MainColor]_BaseColor("Color", Color) = (1,1,1,1)
        _RefractIntensity("RefStrength(宝石反射强度)", Range(0.0,2.0)) = 0.5
        _EnvironmentIntensity("EnvLight(环境反射强度)", Range(0.0,2.0)) = 1.0

        _EmissionMap("EmissionTex", 2D) = "white" {}
        [HDR]_EmissionColor("Emission(颜色亮度)", Color) = (1,1,1,1)
    ...
    SubShader{
        ...
        Pass {
            ...
            #pragma fragment GemLitFragment
