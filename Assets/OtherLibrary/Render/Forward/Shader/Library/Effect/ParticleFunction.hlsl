#ifndef PARTICLESFUN_INCLUDE
#define PARTICLESFUN_INCLUDE
...
inline float2 UvRotatorAngle2(float2 uv, float rotatorAngle) {

    float DiffuseMaskAng_cos;
    float DiffuseMaskAng_sin;
    sincos(0.0174 * rotatorAngle, DiffuseMaskAng_sin, DiffuseMaskAng_cos);
    //uv旋转前将旋转中心移到uv中心
    float2 panner = mul(uv - float2(0.5,0.5),
            float2x2(DiffuseMaskAng_cos, -DiffuseMaskAng_sin, DiffuseMaskAng_sin, DiffuseMaskAng_cos)) + float2(0.5,0.5) /* + jxTime * float2(_USpeed_diffusem, _VSpeed_diffusem) */;
...
//repeatU (0 or 1): 1:U方向的WrapMode=Repeat
//repeatV (0 or 1): 1:V方向的WrapMode=Repeat
inline float2 GetUV(float2 uv,float clampValue,float repeatU, float repeatV,float4 _Texture_ST,float4 _Texture_TexelSize) {
    float halfTexelSizeU=_Texture_TexelSize.x*0.5;
    float halfTexelSizeV=_Texture_TexelSize.y*0.5;
    uv=uv*_Texture_ST.xy+_Texture_ST.zw;
    float2 uvDistortClamp = ClampUV(uv);
    float2 uvDistortRepeat = frac(uv);
    uvDistortRepeat.x = lerp(uvDistortClamp.x, uvDistortRepeat.x, repeatU);
    uvDistortRepeat.y = lerp(uvDistortClamp.y, uvDistortRepeat.y, repeatV);
    float2 res = lerp(uvDistortClamp, uvDistortRepeat, clampValue);
    res=clamp(res,float2(halfTexelSizeU,halfTexelSizeV),float2(1.0-halfTexelSizeU,1.0-halfTexelSizeV));
    return res;
}
