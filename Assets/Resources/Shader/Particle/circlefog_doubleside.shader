...
//替换 "bud/downtown/circlefog_doubleside" ,需要确认后续此shader是否还需要
Shader "NewRender/Particle/circlefog_doubleside"
{	
	Properties
	{
		[Header(Blend Mode)]
		...
		//
		_NearFogClip("NearFogClip", Range( 0.2 , 5)) = 1
		_CtN("CtN", Range( 1 , 16)) = 1
		_FarFogClip("FarFogClip", Range( 0.2 , 1)) = 1
		_CtF("CtF", Range( 1 , 16)) = 1
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 ParticleFrag(Varyings_Particle input) : SV_Target
			{
				...
				//
				half3 viewDirObject = input.viewDirObject;
				half foffset = ( viewDirObject.z + _Foffset );
				half exA = Ex( foffset , _NearFogClip , _CtN , 0.1 , 1.0 );
				half exB = Ex( foffset , _FarFogClip , _CtF , 1.0 , -1.0 );
				baseTex.a = saturate( (smStepValue * pow( saturate( ( ( 1.0 - ( viewDirObject.y * 0.01 ) ) + _Height ) ) , 8.0 ) * ( 1.0 - saturate( max( exA , exB ) ) ) ) );
